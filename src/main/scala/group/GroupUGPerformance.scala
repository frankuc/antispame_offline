...
class GroupUGPerformance extends  Serializable {
  ...
  def sqlQuery(): String = {
    ...
    val groupQuery:String =
      s"""
        |WITH ug_performance AS (
        |    SELECT  tb1.ad_package_name AS ad_package_name,
        |            tb1.ad_app_name AS ad_app_name,
        |            tb1.app_id AS app_id,
        ...
        |    FROM    (
        |                SELECT  date,
        |                        get_json_object(extra_json, '$$.app_id') AS app_id,
        |                        get_json_object(extra_json, '$$.rit') AS rit,
        |                        get_json_object(extra_json, '$$.ad_app_name') AS ad_app_name,
        ...
        |                FROM    dm_ad_antispam.antispam_m_ug_activation_performance_di
        |                WHERE   date = '${date}'
        |            ) tb1
        |    INNER JOIN
        |            (
        ...
        |    )
        |),
        |group_ug_performance AS (
        |    SELECT  T.group_schema AS group_schema,
        |            T.group_pattern AS group_pattern,
        |            count(DISTINCT union_imei) AS ug_uimei_cnt,
        |            SUM(activation_cnt) AS ug_activation_cnt,
        |            SUM(st_1800_expect_0) / SUM(st_1800_activation_0) AS ug_day_0_scale,
        |            SUM(st_1800_expect_1) / SUM(st_1800_activation_1) AS ug_day_1_scale,
        |            SUM(st_1800_expect_2) / SUM(st_1800_activation_2) AS ug_day_2_scale,
        |            SUM(st_1800_expect_10) / SUM(st_1800_activation_10) AS ug_day_1_10_scale,
        |            SUM(st_1800_expect_3_10) / SUM(st_1800_activation_3_10) AS ug_day_3_10_scale,
        |            SUM(st_1800_expect_3_30) / SUM(st_1800_activation_3_30) AS ug_day_3_30_scale,
        |            SUM(st_1800_expect_30) / SUM(st_1800_activation_30) AS ug_day_1_30_scale
        |    FROM    (
        ...
        |                                    ) t AS group_schema,
        |                                    group_pattern
        |                            WHERE   ad_app_name IN ('aweme', 'news_article')
        |                        ) t1
        |                INNER JOIN
        |                        (
        |                            SELECT  CAST(app_id AS STRING) AS app_id
        |                            FROM    reward_app_id
        |                            GROUP BY
        |                                    app_id
        |                        ) t3
        ...
        |            T.group_pattern
        |)
        |INSERT OVERWRITE TABLE union_anti.ads_union_antispam_group_aggregator_ad_ug_performance_feature_daily PARTITION (date = '${date}', version = '${version}')
        |SELECT  group_schema,
        |        group_pattern,
        |        TO_JSON(
        |            STRUCT(
